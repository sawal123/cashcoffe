<div x-data="{ showCartMobile: false }"
    class="w-full sm:max-w-sm lg:w-[350px] shrink-0 order-2 lg:order-2 lg:sticky lg:top-24 z-10 h-fit">
    ...
    <style>
        ...
        /* Proteksi tambahan untuk desktop agar CSS mobile tidak bocor */
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
            ...
            /* Warna Tailwind slate-300 untuk light mode */
            border-radius: 10px;
        }

        .dark .custom-scrollbar::-webkit-scrollbar-thumb {
            background-color: #475569;
            /* Warna Tailwind slate-600 untuk dark mode */
        }
        ...
        <div class="sm:hidden fixed bottom-0 left-0 right-0 p-3.5 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shadow-[0_-8px_15px_-3px_rgba(0,0,0,0.1)] z-40 transition-transform duration-300"
            ...
            <div class="flex justify-between items-center max-w-lg mx-auto gap-3">
                <div class="flex flex-col">
                    ...
                    <span class="text-[15px] font-bold text-blue-700 dark:text-blue-400 leading-tight">Rp
                        {{ number_format($totalAfterDiscount, 0, ',', '.') }}</span>
                </div>

                <button type="button" @click="showCartMobile = true"
                    class="flex-1 max-w-[180px] bg-orange-500 hover:bg-orange-600 text-white font-bold h-11 rounded-xl flex items-center justify-center gap-1.5 transition duration-300 shadow-md">
                    <iconify-icon icon="solar:cart-large-minimalistic-bold" class="text-xl"></iconify-icon>
                    <span class="text-sm">Lihat Pesanan</span>
                    @if(count($pesanan) > 0)
                        <span
                            class="bg-white text-orange-600 font-bold text-[10px] px-1.5 py-0.5 rounded-full ml-1">{{ count($pesanan) }}</span>
                    @endif
                </button>
            </div>
        </div>
    ...
    <div id="pesan"
        class="border border-slate-200 mt-2 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 shadow-sm p-4 h-fit max-h-[calc(100vh-1rem)] lg:max-h-[calc(100vh-7rem)] overflow-y-auto custom-scrollbar transition-all duration-300 relative"
        ...
        <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-200 dark:border-slate-700"
            ...
            <div>
                ...
                <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ count($pesanan) }} Item
                    Dipilih</span>
            </div>
            <button type="button" @click="showCartMobile = false; setTimeout(() => { showCartMobile = false }, 100)"
                class="w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 shadow-sm rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition relative z-10">
                <iconify-icon icon="lucide:x" class="text-xl"></iconify-icon>
            </button>
        </div>

        <div class="flex items-center justify-between mb-4 mt-1" x-show="!showCartMobile">
            <p class="font-bold text-lg text-slate-900 dark:text-white">Ringkasan Pesanan</p>
            <span
                class="inline-flex items-center rounded-md bg-orange-500 text-white px-2.5 py-1 text-xs font-semibold">
                {{ count($pesanan) }} Item
            </span>
        </div>
        ...
        <ul class="space-y-2 mb-4 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
            @forelse ($pesanan as $index => $p)
                <li
                    class="flex items-center justify-between gap-3 bg-white dark:bg-slate-800 rounded-xl p-2.5 border border-slate-200/60 dark:border-slate-700">
            ...
            @empty
                <li
                    class="flex flex-col items-center justify-center gap-2 text-center py-10 bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                    <iconify-icon icon="solar:cart-large-minimalistic-linear"
                        class="text-4xl text-slate-300"></iconify-icon>
        ...
        <div x-data="{ expanded: false }"
            class="bg-[#F4F7FB] dark:bg-slate-800 rounded-xl p-3 mb-4 border border-slate-100 dark:border-slate-700">
            <div @click="expanded = !expanded"
                ...
                <iconify-icon :icon="expanded ? 'lucide:chevron-up' : 'lucide:chevron-down'"
                    class="text-slate-400 text-sm transition-transform duration-200"></iconify-icon>
            </div>
            ...
            <div class="grid grid-cols-4 gap-1.5">
                @foreach ($pembayaran as $pay)
                    @php
                        ...
                        if (strtolower($pay) == 'debit' || strtolower($pay) == 'kartu')
        ...
        <div x-data="{ openModal: null }" class="space-y-3 mb-4">
            ...
            <div x-show="openModal !== null" style="display: none;"
                ...
                <div @click.away="openModal = null"
                    class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-sm p-5 shadow-2xl">
                    <div x-show="openModal === 'voucher'">
                        ...
                        @if (isset($disc) && $disc['type'] === 'private' && !$isDiscountVerified)
                            <button type="button"
                                @click="$dispatch('open-modal', { name: 'verify-discount-modal' }); openModal = null"
            ...
            <div class="flex gap-2 items-center">
                @if ($status !== 'selesai')
                    @if ($discount && ($disc === null || $disc['type'] === 'general' || ($disc['type'] === 'private' && $isDiscountVerified)))
                        <div
                            class="flex-1 flex items-center justify-between px-3 h-10 bg-green-50 rounded-lg border border-green-200">
            ...
            <div x-data="{
                ...
                <div x-show="open" style="display: none;"
                    ...
                    <div @click.away="open = false"
                        class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-sm p-5 shadow-2xl relative">
                        ...
                        <x-inputsmall label="Jumlah Uang" x-model="uang" x-on:input="sync" inputmode="numeric" type="text"
                            placeholder="Contoh: 50000" />

                        <div class="mt-1 min-h-[20px] mb-2">
                            <span x-show="!isValid" class="text-[11px] text-red-500 italic block">
                                *Uang tunai masih kurang (Min. Rp {{ number_format($totalAfterDiscount, 0, ',', '.') }})
                            </span>
                        </div>

                        <button type="button" @click="if(isValid) open = false"
                            ...
                            :disabled="!isValid" class="w-full font-bold h-11 rounded-xl transition duration-300 text-sm">
                            Simpan Nominal
                        </button>
                    </div>
                </div>

                <div class="p-3.5 mt-2 bg-blue-50/50 dark:bg-slate-800/80 rounded-xl border border-blue-100 dark:border-slate-700 flex justify-between items-center cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-700 transition"
                    @click="open = true">
                    <span class="text-sm font-semibold text-slate-600 dark:text-slate-400">Uang Tunai:</span>
                    <div class="flex items-center gap-1.5">
                        <span class="text-[15px] font-bold text-blue-700 dark:text-blue-400"
                            x-text="uang ? 'Rp ' + new Intl.NumberFormat('id-ID').format(uang) : 'Rp 0'"></span>
                        <iconify-icon icon="solar:pen-new-square-linear"
                            class="text-lg text-blue-500 dark:text-blue-400"></iconify-icon>
                    </div>
                </div>
            </div>
        @endif
