<div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-8">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 flex items-center justify-center">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 flex items-center justify-center">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 rounded-2xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 flex items-center justify-center">
    ...
    <x-ui.table :headers="['#', 'Karyawan', 'Status Hari Ini', 'Jam Masuk', 'Jam Keluar', ['name' => 'Aksi', 'align' => 'center']]">
        @forelse ($users as $user)
            ...
            @endphp
            <tr wire:key="row-karyawan-{{ $user->id }}"
                class="hover:bg-neutral-50/50 dark:hover:bg-neutral-900/30 transition-colors group">
                ...
                <td data-label="Karyawan" class="px-6 py-5">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-full bg-neutral-100 dark:bg-neutral-700 flex items-center justify-center font-bold text-neutral-500 uppercase">
                            {{ substr($user->name, 0, 2) }}
                        </div>
                        <div>
                            <span
                                class="block text-sm font-bold text-neutral-800 dark:text-neutral-100">{{ $user->name }}</span>
                ...
                <td data-label="Jam Masuk" class="px-6 py-5">
                    @if ($absen?->jam_masuk)
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-sm font-bold text-neutral-700 dark:text-neutral-300">{{ $absen->jam_masuk }}</span>
                        </div>
                    @else
                ...
                <td data-label="Jam Keluar" class="px-6 py-5">
                    @if ($absen?->jam_keluar)
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                class="text-sm font-bold text-neutral-700 dark:text-neutral-300">{{ $absen->jam_keluar }}</span>
                        </div>
                    @else
                        <span class="text-neutral-300">--:--</span>
                    @endif
    ...
    <x-mdal name="modal-tambah-absen">
        <div class="px-6 py-4">
            ...
            <p class="text-neutral-500 text-sm text-center mb-6">
                Masukkan data absensi manual untuk karyawan <strong
                    class="text-neutral-800 dark:text-neutral-200">{{ $selectedUserName }}</strong>.
            </p>
            ...
            <form wire:submit.prevent="storeManualAbsen">
                <div class="space-y-4">
                    <div>
                        <label
                            class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Shift</label>
                        <select wire:model="shiftId"
                            ...
                            @foreach ($shifts as $sh)
                                <option value="{{ $sh->id }}">{{ $sh->nama_shift }}
                                    ({{ \Carbon\Carbon::parse($sh->jam_masuk)->format('H:i') }} -
                                    {{ \Carbon\Carbon::parse($sh->jam_keluar)->format('H:i') }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        <label
                            class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Status</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Jam
                                Masuk</label>
                    ...
                    <div>
                        <label
                            class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Keterangan
                            / Catatan</label>
    ...
    <x-mdal name="modal-detail-absen">
        <div class="px-6 py-4">
            ...
            <p class="text-neutral-500 text-sm text-center mb-6">
                Informasi absensi karyawan <strong
                    class="text-neutral-800 dark:text-neutral-200">{{ $selectedUserName }}</strong>.
            </p>
            ...
            <form wire:submit.prevent="updateManualAbsen">
                <div class="space-y-4">
                    @if ($fotoUrl)
                        <div class="flex flex-col items-center mb-4">
                            <label
                                class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Foto
                                Selfie</label>
                            <img src="{{ $fotoUrl }}" alt="Selfie"
                                class="w-32 h-32 object-cover rounded-2xl border-2 border-neutral-200 dark:border-neutral-700 shadow-sm">
                        </div>
                    @endif
                    ...
                    @if ($lokasiStr)
                        <div>
                            <label
                                class="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1.5">Lokasi
                                Map</label>
                            <div
                                class="w-full h-32 rounded-xl overflow-hidden border border-neutral-200 dark:border-neutral-700 relative">
                                @php
                ...
                <div
                    class="mt-8 flex justify-between items-center pt-4 border-t border-neutral-100 dark:border-neutral-700">
                    <button type="button" wire:click="deleteManualAbsen"
                        wire:confirm="Apakah Anda yakin ingin menghapus absensi hari ini?"
    ...
    @if ($showDetailModal && $selectedAbsenDetail)
        <div
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/60 backdrop-blur-sm transition-all duration-300">
            <div
                class="bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-2xl w-full max-w-md md:max-w-lg overflow-hidden transform transition-all scale-100">

                {{-- Modal Header --}}
                <div
                    class="px-6 py-4 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex justify-between items-center">
                ...
                <div class="px-6 py-5 space-y-4 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    {{-- User Profile Card --}}
                    <div
                        class="flex items-center gap-3 p-3 bg-neutral-50 dark:bg-neutral-800/30 rounded-2xl border border-neutral-100 dark:border-neutral-800/50">
                        <div
                            class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-sm uppercase">
                            {{ substr($selectedAbsenDetail->user->name, 0, 2) }}
                        </div>
                        <div>
                            <span
                                class="block font-bold text-neutral-800 dark:text-neutral-100">{{ $selectedAbsenDetail->user->name }}</span>
                            ...
                            <span
                                class="inline-flex px-2 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 text-[10px] font-bold">
                    ...
                    {{-- Shift Schedule Info --}}
                    <div>
                        <span class="block text-xs font-bold text-neutral-400 uppercase tracking-wider mb-1.5">Jadwal
                            Shift Hari Ini</span>
                        <div
                            class="p-3 bg-blue-50/50 dark:bg-blue-950/20 text-blue-800 dark:text-blue-300 rounded-2xl border border-blue-100/50 dark:border-blue-900/30 text-xs font-semibold">
                            <iconify-icon icon="lucide:clock"
                                class="inline-block mr-1 text-sm align-middle"></iconify-icon>
                    ...
                    {{-- Clock-In / Jam Masuk --}}
                    <div>
                        <span class="block text-xs font-bold text-neutral-400 uppercase tracking-wider mb-2">Waktu
                            Realita Lapangan</span>

                        <div class="space-y-4">
                            {{-- Clock In details and maps --}}
                            <div>
                                <div class="flex items-start justify-between mb-1.5">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-6 h-6 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                                            <iconify-icon icon="lucide:log-in" class="text-sm"></iconify-icon>
                                        </div>
                                        <span class="font-semibold text-neutral-800 dark:text-neutral-200">Jam
                                            Masuk</span>
                                    </div>
                                    <div class="text-right">
                                        ...
                                        @if ($isLate)
                                            <span
                                                class="inline-flex px-2 py-0.5 rounded bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 text-[10px] font-extrabold uppercase tracking-wide">
                                                Terlambat {{ $lateMinutes }} Menit
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-[10px] font-extrabold uppercase tracking-wide">
                                                Tepat Waktu
                                            </span>
                                        @endif
                                    </div>
                                ...
                                {{-- Clock In Photo and Location map --}}
                                @if ($selectedAbsenDetail->foto || $selectedAbsenDetail->lokasi)
                                    <div class="grid grid-cols-2 gap-3 pl-8">
                                        @if ($selectedAbsenDetail->foto)
                                            <div
                                                class="relative group overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800">
                                                <img src="{{ asset('storage/' . $selectedAbsenDetail->foto) }}"
                                                    alt="Foto Masuk" class="w-full h-24 object-cover">
                                                <div
                                                    class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                                    <a href="{{ asset('storage/' . $selectedAbsenDetail->foto) }}"
                                                        target="_blank"
                                                        class="text-white text-[10px] bg-neutral-900/85 px-2.5 py-1.5 rounded-lg flex items-center gap-1 font-bold">
                                                        <iconify-icon icon="lucide:maximize-2"
                                                            class="text-[10px]"></iconify-icon> Lihat
                                                    </a>
                                                </div>
                                            </div>
                                        @else
                                            <div
                                                class="w-full h-24 flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-300 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/10">
                                                <iconify-icon icon="lucide:image"
                                                    class="text-neutral-400 text-lg mb-1"></iconify-icon>
                                                <span
                                                    class="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">No
                                                    Photo</span>
                                            </div>
                                        @endif

                                        @if ($selectedAbsenDetail->lokasi)
                                            @php
                                                $coordsMasuk = explode(',', $selectedAbsenDetail->lokasi);
                                                $latMasuk = trim($coordsMasuk[0] ?? '-6.200000');
                                                $lngMasuk = trim($coordsMasuk[1] ?? '106.816666');
                                            @endphp
                                            <div
                                                class="relative group overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800 h-24">
                                                <iframe class="w-full h-full border-0 absolute inset-0" loading="lazy"
                                                    referrerpolicy="no-referrer-when-downgrade"
                                                    src="https://www.google.com/maps?q={{ $latMasuk }},{{ $lngMasuk }}&hl=id&z=14&output=embed"></iframe>
                                                <div class="absolute bottom-1 right-1">
                                                    <a href="https://www.google.com/maps/search/?api=1&query={{ $latMasuk }},{{ $lngMasuk }}"
                                                        target="_blank"
                                                        class="inline-flex items-center justify-center w-6 h-6 rounded-lg bg-neutral-900/80 hover:bg-neutral-950 text-white transition-colors shadow-sm"
                            ...
                            {{-- Clock Out details and maps --}}
                            <div>
                                <div class="flex items-start justify-between mb-1.5">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-6 h-6 rounded-lg bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 flex items-center justify-center">
                                    ...
                                    <div class="text-right">
                                        ...
                                        @if ($selectedAbsenDetail->jam_keluar)
                                            @php
                                                ...
                                                $earlyMinutes = 0;
                                                if ($activeShift) {
                                                    $jamKeluarTime = \Carbon\Carbon::parse(
                                                        $selectedAbsenDetail->jam_keluar,
                                                    );
                                            ...
                                            @if ($isEarly)
                                                <span
                                                    class="inline-flex px-2 py-0.5 rounded bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 text-[10px] font-extrabold uppercase tracking-wide">
                                ...
                                {{-- Clock Out Photo and Location map --}}
                                @if ($selectedAbsenDetail->jam_keluar && ($selectedAbsenDetail->foto_keluar || $selectedAbsenDetail->lokasi_keluar))
                                    <div class="grid grid-cols-2 gap-3 pl-8">
                                        @if ($selectedAbsenDetail->foto_keluar)
                                            <div
                                                class="relative group overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800">
                                                <img src="{{ asset('storage/' . $selectedAbsenDetail->foto_keluar) }}"
                                                    alt="Foto Pulang" class="w-full h-24 object-cover">
                                                <div
                                                    class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                                                    <a href="{{ asset('storage/' . $selectedAbsenDetail->foto_keluar) }}"
                                                        target="_blank"
                                                        class="text-white text-[10px] bg-neutral-900/85 px-2.5 py-1.5 rounded-lg flex items-center gap-1 font-bold">
                                                        <iconify-icon icon="lucide:maximize-2"
                                                            class="text-[10px]"></iconify-icon> Lihat
                                        ...
                                        @if ($selectedAbsenDetail->lokasi_keluar)
                                            @php
                                                $coordsKeluar = explode(',', $selectedAbsenDetail->lokasi_keluar);
                                                $latKeluar = trim($coordsKeluar[0] ?? '-6.200000');
                                                $lngKeluar = trim($coordsKeluar[1] ?? '106.816666');
                                            @endphp
                                            <div
                                                class="relative group overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-800 h-24">
                                                <iframe class="w-full h-full border-0 absolute inset-0" loading="lazy"
                                                    referrerpolicy="no-referrer-when-downgrade"
                                                    src="https://www.google.com/maps?q={{ $latKeluar }},{{ $lngKeluar }}&hl=id&z=14&output=embed"></iframe>
                                                <div class="absolute bottom-1 right-1">
                                                    <a href="https://www.google.com/maps/search/?api=1&query={{ $latKeluar }},{{ $lngKeluar }}"
                                                        target="_blank"
                                                        class="inline-flex items-center justify-center w-6 h-6 rounded-lg bg-neutral-900/80 hover:bg-neutral-950 text-white transition-colors shadow-sm"
                                                        title="Buka Google Maps">
                                                        <iconify-icon icon="lucide:map-pin"
                    ...
                    <div
                        class="flex items-center justify-between p-3 bg-red-50/50 dark:bg-rose-950/10 border border-red-100/50 dark:border-rose-900/20 rounded-2xl">
                ...
                {{-- Modal Footer --}}
                <div
                    class="px-6 py-4 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-900/50 flex justify-end">
                    <button type="button" wire:click="closeDetailModal"
